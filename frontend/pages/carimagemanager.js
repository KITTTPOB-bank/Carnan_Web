...
import { SearchOutlined, UnorderedListOutlined, CarOutlined, EditOutlined, DownloadOutlined } from "@ant-design/icons";
...
const carimagemanager = () => {
    ...
    const fetchDatacar = async () => {
        try {
            const apiUrl = 'https://35.175.230.48/cardataadmin';
            ...
            render: (_, record) => {
                return (
                    <Space size={'large'}>
                        ...
                        <Tooltip placement="top" title={'แก้ไขรูปภาพ'} onClick={() => Nextimagespage(record)}>
                            <Typography.Link style={{ display: 'flex', alignItems: 'center', fontSize: 20 }}>
                                <EditOutlined />
                            </Typography.Link>
                        </Tooltip>
    ...
    const downloadimage = async (record) => {
        ...
        try {
            ...
            const apiUrl = "https://35.175.230.48/downloadfile";
            const queryParams = {
                brand: brand,
                name: name,
                year: year,
